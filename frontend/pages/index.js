...
const [thinking, setThinking] = useState(false);
  ...
  async function daziV2() {
    ...
    if (!wtDaziPathHasEnemy(state, path)) {
      setError("Disarm needs a word that includes at least one purple enemy letter.");
      return;
    }
try {
      const payload = { path };
        ...
        <div className="bcol">
          ...
          {!state.winner && <div className="mpanel">
            <div className="mrow">
              ...
              <button className="ba" onClick={pass} disabled={!human() || daziMode || rotateMode}>Pass</button>
            </div>
          </div>}
        </div>
